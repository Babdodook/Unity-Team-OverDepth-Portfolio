...
public class FanaticMaceController : BaseMonsterController
{
    [HideInInspector]
    ...
    IEnumerator Start()
    {
        ...
        if (Static_Data.m_number == 1)
        {
            try
            {
                TCPClient.m_Monster.Monster_BeginInfoUpdate(Packing.STATE.FANATIC_MASE, index, sc_Health.maxHP);
            }
            catch { }
        }
    }
    ...
    protected override void Update()
    {
        ...
        UpdateMoveValue();
        OnHitCheck();
        CheckOtherPlayerExist(Packing.STATE.FANATIC_MASE);
        SetGravity();
        //CheckObstacle();
    }
    ...
    protected override void CheckAttackDistance()
    {
        ...
        if (TargetDistance > battleDistance)
        {
            if (m_Animator.GetInteger("BattleSTATE") != (int)AnimBattleSTATE.Move)
            {
    ...
    void SetMovement()
    {
        if (!isMoving)
        {
            ...
            if(TargetDistance <= 3f)
            {
                ...
                if (RandomAction >= 50)
                {
                    float RandomTime = UnityEngine.Random.Range(2.0f, 2.5f);
    ...
    public void PlayBattleAction(FanaticBattleType actionType)
    {
        switch (actionType)
        {
            ...
            case FanaticBattleType.Attack2:
                ...
                currentAttackType = 0;
                CreateRandomActionTime(10f, 15f);
    ...
    public void OnHitCheck()
    {
        ...
        if (m_prevHp > sc_Health.curHP)
        {
            ...
            if (sc_Health.curHP <= 0)
            {
                try
                {
                    TCPClient.m_Monster.Monster_Die(Packing.STATE.FANATIC_MASE, index, transform.position, transform.rotation);
                }
                catch { }
            ...
            else if (sc_Health.curHP <= sc_Health.maxHP)
            {
                ...
                {
                    print("메이스 피격 보냄");
                    TCPClient.m_Monster.Monster_Hit(Packing.STATE.FANATIC_MASE, index, transform.position, transform.rotation, onHitRandomAction, damage, currentOnHitType);
                }
                catch { }
    ...
    public void OnHit_State(Vector3 pos, Quaternion rot, int anim, int hp, int hitType)
    {
        ...
        print("피격 번호: " + hitType + " 회피 번호: " + onHitRandomAction);
