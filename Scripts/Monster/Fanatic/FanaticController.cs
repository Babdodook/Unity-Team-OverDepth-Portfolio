...
public class FanaticController : BaseMonsterController
{
    [HideInInspector]
    ...
    IEnumerator Start()
    {
        ...
        if (Static_Data.m_number == 1)
        {
            try
            {
                TCPClient.m_Monster.Monster_BeginInfoUpdate(Packing.STATE.FANATIC_SWORD, index, sc_Health.maxHP);
            }
            catch { }
        }
    }
    ...
    protected override void Update()
    {
        ...
        UpdateMoveValue();
        OnHitCheck();
        CheckOtherPlayerExist(Packing.STATE.FANATIC_SWORD);
        SetGravity();
        //CheckObstacle();
    }
    ...
    protected override void UpdateRotateSpeed()
    {
        if (!LockRotation)
        {
            if (Target != null)
            {
                lookrotation = Target.position - transform.position; // Search_Path - transform.position;
                lookrotation.y = 0;
    ...
    protected override void CheckAttackDistance()
    {
        ...
        else if (TargetDistance <= battleDistance)
        {
            ...
            if (Time.time - prevTime >= RandomActionTime)
            {
                ...
                if (TargetDistance <= AttackDistance)
                {
                    if (RandomAction >= 70)
                        ...
                        m_FanaticBattleType = FanaticBattleType.Attack4;
            ...
            if (EvadeDelayTime >= 15f && TargetDistance >= AttackDistance)
            {
                // 타겟이 공격상태일때 10퍼센트 확률로 회피
                if (Target.GetComponentInChildren<PlayerAnimControl>().Anim.GetBool("Attacking"))
                {
                    if (10 <= RandomAction && RandomAction <= 20)
                    {
                        RandomAction = UnityEngine.Random.Range(0, 100);
                        if (RandomAction > 50)
                            m_FanaticBattleType = FanaticBattleType.Evade;
                        else
                            m_FanaticBattleType = FanaticBattleType.Evade2;
                        EvadeDelayTime = 0;
                        ...
                        //print("회피 실행 RandomAction: " + RandomAction);
                    }
                }
            }
        }
    ...
    public void PlayBattleAction(FanaticBattleType actionType)
    {
        switch (actionType)
        {
            case FanaticBattleType.Evade:
                ...
                m_FanaticBattleType = FanaticBattleType.Max;
                break;
            case FanaticBattleType.Evade2:
                //print("회피기 실행");
                m_isEvade = true;
                isMoving = false;
                TCP_setAnimation = false;
            ...
            case FanaticBattleType.Attack4:
                ...
                m_Animator.SetInteger("BattleSTATE", (int)AnimBattleSTATE.Attack4);
                break;
    ...
    public void OnHitCheck()
    {
        ...
        if (m_prevHp > sc_Health.curHP)
        {
            ...
            if (sc_Health.curHP <= 0)
            {
                try
                {
                    TCPClient.m_Monster.Monster_Die(Packing.STATE.FANATIC_SWORD, index, transform.position, transform.rotation);
                }
                catch { }
            ...
            else if (sc_Health.curHP <= sc_Health.maxHP)
            {
                ...
                onHitRandomAction = UnityEngine.Random.Range(0, 2);
                try
                {
                    TCPClient.m_Monster.Monster_Hit(Packing.STATE.FANATIC_SWORD, index, transform.position, transform.rotation, onHitRandomAction, damage, currentOnHitType);
                }
                catch { }
    ...
    public void OnHit_State(Vector3 pos, Quaternion rot, int anim, int hp, int hitType)
    {
        ...
        //print("피격번호: " + hitType + " 회피 번호: " + onHitRandomAction);
        m_Animator.SetInteger("OnHit", hitType);
    }
    ...
    void AnimationMovement()
    {
        ...
        if (EvadeTranslate)
        {
            ...
            switch (m_evadeType)
            {
                case EvadeType.backward:
                    direction = transform.forward * -1;
                    break;
                case EvadeType.right:
                    LockRotation = false;
    ...
    public void TCP_SendTarget()
    {
        ...
        if (SetTarget_FrameCheck == 0)
        {
            ...
            catch
            {
                ...
                {
                    case MONSTER_STATE.IDLE:
                        m_Animator.SetInteger("IdleSTATE", (int)AnimIdleSTATE.GetWeapon);
                        SetMoveType(MoveType.WalkForward);
                        //m_moveType = MoveType.WalkForward;
    ...
    public bool TCP_SendMovement(float speed)
    {
        ...
        {
            FrameCheck = -1;
            var desiredPosition = transform.position + (m_desiredMoveDirection); //* speed * Time.deltaTime * 5);
    ...
    public void TCP_RecvTarget(int classNumber)
    {
        ...
        if(m_nowState == MONSTER_STATE.IDLE)
            m_Animator.SetInteger("IdleSTATE", (int)AnimIdleSTATE.GetWeapon);
        SetMoveType(MoveType.WalkForward);
        m_nowState = MONSTER_STATE.TRACE;
